...
ADD https://www.urbackup.org/downloads/Server/$(curl https://www.urbackup.org/downloads/Server/ | grep -oP 'href="\K[0-9]+\.[0-9]+\.[0-9]+' | sort -t. -rn -k1,1 -k2,2 -k3,3 | head -1)/debian/stretch/urbackup-server_$(curl https://www.urbackup.org/downloads/Server/ | grep -oP 'href="\K[0-9]+\.[0-9]+\.[0-9]+' | sort -t. -rn -k1,1 -k2,2 -k3,3 | head -1)_amd64.deb /root/urbackup.deb
